...
def _parse_koncorde_alert_nl(text: str) -> dict | None:
    ...
    system = (
        ...
        '"la flecha azul/verde gira al alza"/"blue empieza a subir"/"el verde se da la vuelta arriba" '
        '-> blue_turns_up / green_turns_up; '
        '"la línea marrón (tendencia) gira a la baja"/"el trend se da la vuelta abajo" -> trend_turns_down; '
        '"blue subiendo"/"flecha verde hacia arriba"/"la tendencia va hacia arriba" '
        '-> blue_rising / green_rising / trend_rising (usa *_falling para "bajando"); '
        '"cruce al alza de la línea roja"/"la línea negra cruza la roja hacia arriba"/'
        '"el trend cruza su media al alza" -> trend_cross_up (usa trend_cross_down para "a la baja"/'
        '"cruza hacia abajo"). OJO: esto es DISTINTO de trend_turns_up/down (que solo miran si la '
        'propia línea de tendencia gira) — "cruce" siempre implica dos líneas cruzándose entre sí '
        '(trend vs su línea roja de señal), no que una línea sola cambie de pendiente.\n'
